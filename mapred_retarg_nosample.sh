...
echo $hstream -input $input_path2 -output ${OUTDIR_HOME} -mapper 'python mapper_retarg.py nosample' -file $dir/mapper_retarg.py -reducer 'cat'  -jobconf mapred.job.name=retarg -jobconf mapred.reduce.tasks=500
$hstream -input $input_path2 -output ${OUTDIR_HOME} -mapper 'python mapper_retarg.py nosample' -file $dir/mapper_retarg.py -reducer 'cat'  -jobconf mapred.job.name=retarg -jobconf mapred.reduce.tasks=500 -jobconf mapred.job.queue.name=science
