...
echo $hstream -input $input_path2 -output ${OUTDIR_HOME} -mapper 'python mapper_ending.py A' -file mapper_ending.py -reducer 'cat' -jobconf mapred.reduce.tasks=500 -jobconf mapred.job.name=yandong_sample_A 
$hstream -input $input_path2 -output ${OUTDIR_HOME} -mapper 'python mapper_ending.py A' -file mapper_ending.py -reducer 'cat' -jobconf mapred.reduce.tasks=500 -jobconf mapred.job.name=yandong_sample_A 
