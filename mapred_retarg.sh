...
printf -v OUTDIR_HOME "/projects/output/merged/retarg/%04d%02d%02d-%04d%02d%02d-endA" $year $m1 $d1 $year $m2 $d2
...
echo $hstream -input $input_path2 -output ${OUTDIR_HOME} -mapper 'python mapper_retarg.py' -file mapper_retarg.py -reducer 'cat'  -jobconf mapred.job.name=retarg -jobconf mapred.reduce.tasks=500
$hstream -input $input_path2 -output ${OUTDIR_HOME} -mapper 'python mapper_retarg.py' -file mapper_retarg.py -reducer 'cat'  -jobconf mapred.job.name=retarg -jobconf mapred.reduce.tasks=500
