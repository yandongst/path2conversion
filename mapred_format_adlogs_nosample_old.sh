...
echo $hstream -input "$input_path2" -output ${OUTDIR} -mapper 'python mapper_format_adlogs_oldformat.py nosample' -file $dir/mapper_format_adlogs_oldformat.py -reducer 'cat'  -jobconf mapred.job.name=yandong_event_w_timestamp -jobconf mapred.reduce.tasks=50
$hstream -input "$input_path2" -output ${OUTDIR} -mapper 'python mapper_format_adlogs_oldformat.py nosample' -file $dir/mapper_format_adlogs_oldformat.py -reducer 'cat'  -jobconf mapred.job.name=yandong_event_w_timestamp -jobconf mapred.reduce.tasks=50 -jobconf mapred.job.queue.name=science
